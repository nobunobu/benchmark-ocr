...
import { OCR_SYSTEM_PROMPT } from './shared';
...
export class AWSBedrockProvider extends ModelProvider {
  ...
  constructor(model: string) {
    ...
    this.inferenceProfileId = process.env.AWS_BEDROCK_INFERENCE_PROFILE_LLAMA4 || '';
    
    if (!this.inferenceProfileId) {
      throw new Error('AWS_BEDROCK_INFERENCE_PROFILE_LLAMA4 environment variable is required for Llama 4 Maverick models');
    }
...
JSON:`;
      ...
      try {
        ...
        console.warn('Failed to parse JSON from Llama 4 response:', responseText);
